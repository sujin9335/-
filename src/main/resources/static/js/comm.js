        ...
        if (totalPage < 11) { //10페이지 이하일경우 <> 필요없음
            ...
            for (var i = 1; i < totalPage + 1; i++) {
                if (current == i) { //보고있는 페이지 일경우
                    page += "<li class='paginate_button page-item active'><a class='page-link ' href='#' onclick='changePage(" + i + ");'>" + i + "</a></li>";
                } else {
                    page += "<li class='paginate_button page-item'><a class='page-link' href='#' onclick='changePage(" + i + ");'>" + i + "</a></li>";
                }
            }

            next = "<li class='paginate_button page-item next'>" +
                addNext +
                "</li>" +
                "</ul>" 
                ;
            ...
            for (var i = 1; i < 11; i++) {
                ...
                if ((Number(preChar) * 10 + i) < Number(totalPage) + 1) {
                    if (current == (Number(preChar) * 10 + i)) {
                        page += "<li class='paginate_button page-item'><a class='page-link active' href='#' onclick='changePage(" + (Number(preChar) * 10 + i) + "," + totalPage + ");'>" + (Number(preChar) * 10 + i) + "</a></li>";
                    } else if (i == 10) {
                        page += "<li class='paginate_button page-item'><a class='page-link' href='#' onclick='changePage(" + ((Number(preChar) + 1) * 10) + "," + totalPage + ");'>" + ((Number(preChar) + 1) * 10) + "</a></li>";
                    } else {
                        page += "<li class='paginate_button page-item'><a class='page-link' href='#' onclick='changePage(" + (Number(preChar) * 10 + i) + "," + totalPage + ");'>" + (Number(preChar) * 10 + i) + "</a></li>";
                    }
                } else {
                    break;
                }
