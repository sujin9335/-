...
$.widget( "ui.scrolltable", {
	...
	_create: function(){
		...
		//.st-body-table를 늦게 찾아서 st-head에 padding 안들어가서 수정
		setTimeout(() => {
		var padding = $self.outerWidth() - $self.find('.st-body-table').outerWidth();
		$self.find('.st-head').css('padding-right', padding + 'px');
		}, 1);
	...
	destroy: function(){
		...
		$self.removeClass('st-container')
			.find('>.st-head').replaceWith($self.find('.st-head-table>thead')).end()
			.find('>.st-body').replaceWith($self.find('.st-body-table>tbody')).end()
			.find('.st-head').closest('tr').remove().end()
			.find('.st-body').closest('tr').remove();

		$self.find('>.st-head th, >.st-body td')
			.each(function(){
	...
	_convertTable: function($container){
		...
		$container
			...
			.html('<div class="st-head"></div><div class="st-body"><div class="st-body-scroll"></div></div>')
